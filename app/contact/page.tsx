    ...
    if (firstName && lastName && email && phone) {
      try {
        ...
        if (response.ok) {
          ...
          setErrorMsg(data.error || data.message || "Failed to submit request.");
        }
      } catch (error: any) {
            ...
            <div className="lg:col-span-7 bg-white border border-slate-100 rounded-2xl sm:rounded-[2rem] md:rounded-[2.5rem] p-5 sm:p-8 md:p-12 shadow-xl shadow-slate-200/40 relative overflow-hidden transition-all duration-500">
              ...
              {success ? (
                ...
                <div className="w-full">
                  {errorMsg && (
                    <div className="p-4 mb-6 text-sm text-red-800 bg-red-50 rounded-xl border border-red-100 flex flex-col gap-1 w-full text-left">
                      <span className="font-bold flex items-center gap-1.5 text-red-950">
                        <svg className="w-4 h-4 text-red-600 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                        Mail Dispatch Failed:
                      </span>
                      <p className="text-red-700 text-xs font-normal font-sans leading-relaxed">{errorMsg}</p>
                    </div>
                  )}
