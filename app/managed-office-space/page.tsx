      ...
      try {
        const response = await fetch("http://localhost:8000/api/contact", {
          ...
          body: JSON.stringify({
            ...
            phone: bookingPhone,
            company: "",
            message: `Booking Inquiry for Managed Office: ${bookingLookingFor || selectedPlan}`,
            source: "popup",
            bot_field: botField
          }),
        });
      ...
      >
        <div className="absolute inset-0 z-0">
          <Image
            ...
            alt="Managed Office Space Background"
            fill
            priority
        ...
        <div className="relative z-10 lg:w-1/2 text-left flex flex-col items-start gap-6">
          ...
          <p className="text-slate-300 text-sm sm:text-base leading-relaxed max-w-xl font-normal">
            Covai Tech Park provides fully customizable corporate offices. From workspace architecture to utility management, housekeeping, and premium reception support, we run the office so you can run the business.
          </p>
      ...
      <section className="py-6 bg-white border-t border-b border-slate-200 shadow-sm w-full section-x">
        <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-6 gap-4">
          {[
            ...
            { label: "Contract", val: "Flexible Terms" },
            { label: "Housekeeping", val: "Daily Included" },
            { label: "IT Setup", val: "Dedicated Rack" },
            { label: "Meeting Room", val: "SLA Unlimited" },
            { label: "Branding", val: "Private Custom" }
          ].map((item, idx) => (
      ...
      <section id="amenities" className="py-20 sm:py-28 bg-slate-100 section-x w-full relative border-t border-b border-slate-200">
        <div className="max-w-7xl mx-auto space-y-12 sm:space-y-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
            {MANAGED_OFFICE_AMENITIES.map((amenity, idx) => (
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-lg overflow-hidden border border-slate-100 shadow-2xl relative p-8 sm:p-10 animate-in fade-in zoom-in-95 duration-200">
            ...
            {bookingSuccess ? (
              <div className="py-12 text-center flex flex-col items-center gap-4">
                ...
                <h4 className="font-outfit font-medium text-lg text-slate-800">Inquiry Submitted!</h4>
                <p className="text-slate-400 text-sm font-normal">Our corporate space consultants will contact you shortly.</p>
              </div>
            ) : (
