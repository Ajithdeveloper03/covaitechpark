...
// Replace Gallery Section in app/page.tsx
const galleryStart = '{/* GALLERY SECTION — Masonry scroll strip with white edge overlays + glassmorphism */}';
const galleryEnd = '{/* FAQS SECTION */}';

const startIndex = contentPage.indexOf(galleryStart);
const endIndex = contentPage.indexOf(galleryEnd);

if (startIndex !== -1 && endIndex !== -1) {
    const newGallery = `{/* GALLERY SECTION — Masonry scroll strip with white edge overlays + glassmorphism */}
      <section id="gallery-works" className="bg-slate-50 py-16 sm:py-24 w-full text-brand-navy relative overflow-hidden">
        <div className="w-full flex flex-col items-center relative z-10">
          
          {/* Header block - Centered */}
          <div className="flex flex-col items-center justify-center gap-4 mb-12 px-4 sm:px-8 max-w-4xl mx-auto w-full text-center">
            <span className="text-[10px] font-bold text-brand-orange uppercase tracking-[0.25em] flex items-center justify-center gap-2">
              <span className="w-6 h-[1px] bg-brand-orange hidden sm:block"></span> OUR GALLERY <span className="w-6 h-[1px] bg-brand-orange hidden sm:block"></span>
            </span>
            ...
            <p className="text-slate-500 text-sm font-normal max-w-2xl leading-relaxed mt-2">
          ...
          {/* Carousel container */}
          <div className="relative w-full overflow-hidden mt-6 pb-12">
            ...
            <div className="overflow-hidden w-full" ref={galleryRef}>
              <div className="flex gap-6 sm:gap-8 items-start px-4 sm:px-12 pt-4 pb-20">
                {GALLERY_ITEMS.map((item, index) => {
                  return (
                    <div key={index} className={\`flex-[0_0_260px] sm:flex-[0_0_320px] lg:flex-[0_0_380px] min-w-0 transition-transform duration-700 ease-out \${index % 2 !== 0 ? 'translate-y-12 sm:translate-y-16' : ''}\`}>
                      
                      {/* Image Card - Fixed Size */}
                      <div className="group relative w-full aspect-[4/5] rounded-[2rem] overflow-hidden shadow-sm hover:shadow-xl hover:border-brand-orange/30 border border-transparent transition-all duration-500 cursor-pointer mb-5">
                        <Image 
                          src={item.image} 
                          alt={item.title} 
                          fill 
                        ...
                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-brand-navy/30">
      ...
      `;
    contentPage = contentPage.substring(0, startIndex) + newGallery + contentPage.substring(endIndex);
    fs.writeFileSync(pathPage, contentPage, 'utf8');
    console.log('Homepage Gallery updated.');
} else {
    console.log('Could not find Gallery section in page.tsx');
}
